...
class Solution:
    def getDecimalValue(self, head: ListNode) -> int:
        def helper(head: ListNode, count: int) -> int:
            if head is None:
                return count
            newcount = (count * 2) + head.val
            return helper(head.next, newcount)
...
if __name__ == "__main__":
    ...
    want = 5
    got = Solution().getDecimalValue(l)
    print(f"pass? [{want == got}] want[{want}] got[{got}]")
