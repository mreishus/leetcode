...
class Solution:
    def maxProfit(self, prices: List[int]) -> int:
        max_profit = 0
        for price, prev in zip(prices[1:], prices):
            this_profit = price - prev
            max_profit += max(0, this_profit)
        return max_profit


if __name__ == "__main__":
    print("122 - Best time to buy and sell stock ii")
    cases = [([7, 1, 5, 3, 6, 4], 7), ([1, 2, 3, 4, 5], 4), ([7, 6, 4, 3, 1], 0)]
    for (data, want) in cases:
        got = Solution().maxProfit(data)
        print(f"Pass? {want == got} | Want {want} | Got {got} | Data {data}")
