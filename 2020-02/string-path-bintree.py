...
class Solution:
    def isValidSequence(self, root: TreeNode, arr: List[int]) -> bool:
        def helper(root: TreeNode, arr: List[int], i: int) -> bool:
            if root is None or i >= len(arr) or arr[i] != root.val:
                return False

            if root.left is None and root.right is None:
                return i == (len(arr) - 1)

            return helper(root.left, arr, i + 1) or helper(root.right, arr, i + 1)
